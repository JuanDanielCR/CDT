...
public class Aspirante implements Modelo{
	...
	/*
	 * Relacion ManyToMany, referencia a la tabla intermedia donde se vaciará
	 * la información, así como la llaves transferidas, esta relacion solo se 
	 * hace en la entidad que lo usara, no hay necesidad de crear una lista
	 * en cada entidad, solo si el caso de uso lo necesita
	 * */
	@ManyToMany
	@JoinTable(name="tad19_aspirante_direccion",
		joinColumns = {@JoinColumn(name="id_aspirante", updatable=false, insertable=false)},
		inverseJoinColumns={@JoinColumn(name="id_direccion", referencedColumnName="id_direccion",updatable=false, insertable=false)})
